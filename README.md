...
**Version**: 3.1
**Target Environment**: Claude.ai (chat) / Claude Code / その他 Claude SK 対応環境
**API Method**: `window.claude.complete()` (Claude.ai 規約準拠)
**License**: MIT
...
1. `.claude/skills/council/` 以下にこのリポジトリの中身を配置
2. SKが自動認識される
3. 「councilで〇〇について相談したい」と話しかける

詳細は `references/installation.md` を参照。
...
このSKは個人意思決定支援を目的とした思想実験である。バグや改善提案は歓迎する。
